...
def hole_aktuellen_szr_kurs():
    ...
    try:
        ...
        print(f"API-Error: {e}") # Nur für dich zum Debuggen
...
def berechne_haftung():
    try:
        ...
        ergebnis_text = f"Limit of liability: {haftungsgrenze:.2f} €\n"
        
        if warenwert > haftungsgrenze:
            label_ergebnis.config(text=ergebnis_text + f"⚠️ UNDERCOVERAGE: {differenz:.2f} € Risk!", fg="red")
        else:
            label_ergebnis.config(text=ergebnis_text + "✅ Everything is covered. No risk.", fg="green")
...
# Überschrift
label_titel = tk.Label(root, text="CMR Liability Auditor", font=("Arial", 16, "bold"))
label_titel.pack(pady=10) # pady ist der Abstand nach oben/unten

# Eingabe Gewicht
label_gewicht = tk.Label(root, text="Gross weight (kg):")
label_gewicht.pack()
entry_gewicht = tk.Entry(root)
entry_gewicht.pack(pady=5)

# Eingabe Warenwert
label_wert = tk.Label(root, text="Value of goods (€):")
label_wert.pack()
entry_wert = tk.Entry(root)
entry_wert.pack(pady=5)

label_kurs = tk.Label(root, text="Current SDR rate:")
label_kurs.pack()
...
if automatischer_kurs:
    ...
    label_kurs.config(text=f"Current SDR rate (Auto-Loaded):")
else:
...
btn_calc = tk.Button(root, text="Check", command=berechne_haftung, bg="lightgrey", font=("Arial", 10))
